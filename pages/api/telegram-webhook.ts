...
const RATE_LIMIT_MESSAGE = 'Вы превысили лимит: 10 голосовых в час. Попробуйте позже.'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  // 1) Validate secret
  const headerToken = (req.headers['x-telegram-bot-api-secret-token'] || req.headers['X-Telegram-Bot-Api-Secret-Token']) as string
  ...
  try {
    ...
    if (!chatId || !messageId) {
      logEvent('webhook_invalid_payload', { chat_id: chatId, message_id: messageId, reason: 'missing_chat_or_message_id' })
      return
    }
    if (!userId) {
      logEvent('webhook_missing_user', { chat_id: chatId, message_id: messageId })
      return
    }
    logEvent('webhook_processing_start', { chat_id: chatId, user_id: userId, message_id: messageId })
    ...
    if (!fileId) {
      logEvent('webhook_invalid_payload', { chat_id: chatId, message_id: messageId, reason: 'missing_file_id' })
      return
    }
    ...
    const payload = { chat_id: chatId, message_id: messageId, file_id: fileId, duration, user_id: userId }
    const useQueue = (process.env.QSTASH_ENABLED ?? 'false') === 'true' && !!process.env.QSTASH_URL
    ...
    if (useQueue) {
      ...
      if (enqueueOk) {
        ...
        logError('enqueue_failed', 'Failed to enqueue job', { chat_id: chatId, user_id: userId, payload })
      }
    } else {
      // Fallback: process inline when queue is disabled
      await sendInitialResponse(chatId)
      await processJob(payload)
    }
  } catch (err) {
    logError('telegram_webhook_error', (err as Error).message, { stack: (err as Error).stack })
  }
}
